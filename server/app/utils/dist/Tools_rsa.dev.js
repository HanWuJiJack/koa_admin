...
var _require = require(path.join(process.cwd(), "./config/logger")),
    logger = _require.logger; //512bit支持的最长明文长度是：512/8 - 11 = 53
...
exports.generator = function generator() {
  if (!fs.existsSync(path.resolve(__dirname, './private.pem'))) {
    ...
    fs.writeFile(path.resolve(__dirname, './public.pem'), publicPem, function (err) {
      if (err) throw err;
      logger.info('公钥已保存！');
    });
    fs.writeFile(path.resolve(__dirname, './private.pem'), privatePem, function (err) {
      if (err) throw err;
      logger.info('私钥已保存！');
    });
  }
...
exports.decrypt = function decrypt(txt) {
  var _priKey = fs.readFileSync(path.resolve(__dirname, './private.pem'));
